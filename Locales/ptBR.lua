...
L["SweepyBoop can keep selected focus macros pointed at the enemy healer in arenas. To opt in, add the letters SBM anywhere in the macro name, for example \"SBM Cyclone\" or \"Focus Kick SBM\".\n\nWhen arena specializations are available, the addon finds the enemy healer and rewrites managed target tokens in those macros from @focus to @arena1, @arena2, or @arena3. Outside arenas, or when no healer can be detected, the addon rewrites those same managed targets back to @focus."] = "SweepyBoop pode manter macros de foco selecionadas apontadas para o curador inimigo em arenas. Para ativar, adicione as letras SBM em qualquer lugar no nome da macro, por exemplo \"SBM Cyclone\" ou \"Focus Kick SBM\".\n\nQuando as especializações da arena estão disponíveis, o addon encontra o curador inimigo e reescreve os tokens de alvo gerenciados nessas macros de @focus para @arena1, @arena2 ou @arena3. Fora de arenas, ou quando nenhum curador pode ser detectado, o addon reescreve esses mesmos alvos gerenciados de volta para @focus.";
L["How it works"] = "Como funciona";
L["Only macros with SBM in the macro name are edited. The marker is case-sensitive, so \"SBM\" works and \"sbm\" is ignored.\n\nInside those macros, SweepyBoop only changes explicit unit target tokens: @focus and @arena1 through @arena5. Other macro text is left untouched.\n\nMacro editing is blocked during combat by the game client, so updates are applied after combat ends if an arena event occurs while you are in combat."] = "Somente macros com SBM no nome são editadas. O marcador diferencia maiúsculas de minúsculas, então \"SBM\" funciona e \"sbm\" é ignorado.\n\nDentro dessas macros, SweepyBoop altera apenas tokens explícitos de alvo de unidade: @focus e @arena1 até @arena5. Outro texto da macro não é alterado.\n\nA edição de macros é bloqueada durante o combate pelo cliente do jogo, então as atualizações são aplicadas após o fim do combate se um evento de arena ocorrer enquanto você estiver em combate.";
L["Example"] = "Exemplo";
L["Macro name:\nSBM Cyclone\n\nMacro body before entering arena:\n#showtooltip\n/cast [@focus] Cyclone\n\nIf the healer is arena2, SweepyBoop changes it to:\n#showtooltip\n/cast [@arena2] Cyclone\n\nAfter leaving arena, the same macro is restored to @focus."] = "Nome da macro:\nSBM Cyclone\n\nCorpo da macro antes de entrar na arena:\n#showtooltip\n/cast [@focus] Cyclone\n\nSe o curador for arena2, SweepyBoop altera para:\n#showtooltip\n/cast [@arena2] Cyclone\n\nDepois de sair da arena, a mesma macro é restaurada para @focus.";
L["Important details"] = "Detalhes importantes";
L["Use this for macros you want SweepyBoop to manage. Do not put SBM in macro names that should keep a fixed arena target.\n\nThe addon does not create macros for this feature. Create your own macro, include SBM in the macro name, and write it using @focus as the default target."] = "Use isto para macros que você quer que SweepyBoop gerencie. Não coloque SBM em nomes de macros que devem manter um alvo fixo da arena.\n\nO addon não cria macros para este recurso. Crie sua própria macro, inclua SBM no nome da macro e escreva-a usando @focus como alvo padrão.";
L["General"] = "Geral";
